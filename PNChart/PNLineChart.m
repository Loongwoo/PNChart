                ...
                for (NSArray<UIBezierPath *> *paths in _chartPath) {
                    for (UIBezierPath *path in paths) {
                        ...
                        if (pointContainsPath) {
                            if (_delegate && [_delegate respondsToSelector:@selector(userClickedOnLinePoint:lineIndex:)]) {
...
- (void)touchKeyPoint:(NSSet *)touches withEvent:(UIEvent *)event {
    ...
    for (NSUInteger p = 0; p < _pathPoints.count; p++) {
        ...
        for (NSUInteger i = 0; i < (int) linePointsArray.count - 1; i += 1) {
            ...
            if (distance <= 10.0) {
                if (_delegate && [_delegate respondsToSelector:@selector(userClickedOnLineKeyPoint:lineIndex:pointIndex:)]) {
        ...
        */
        ...
        if (self.showLabel) {
            ...
            for (NSUInteger i = 0; i < [self.xLabels count]; i++) {
                NSString *xlabel = self.xLabels[i];
                if (xlabel.length == 0) {
                    continue;
                }
